...
def objective_function(rotation):
    # Calculate the error or some measure of goodness-of-fit
    avgs = data.get_avgs(rotation)
    y_predicted, e,f,g,h = fit_rabi(avgs[2], a[2], b[2], c, d, x)
    print("__________________________________")
    print(np.shape(y_predicted))
    print(np.shape(avgs[2]))
    error = np.sum((avgs[2] - y_predicted)**2)
    return error
